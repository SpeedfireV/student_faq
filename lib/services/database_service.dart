...
class DatabaseService {
  ...
  static Future<List<Meeting>> getMeetings(String groupId, Meeting? lastFetchedMeeting) async {
    try {
      ...
      var orderedMeetings = group.collection(meetings).orderBy("creationDate", descending: true);
  ...
  static Future<Meeting> getMeeting(String groupId, String meetingId) async {
    try {
      ...
      return Meeting.fromJson((await meeting.get()).data()!).copyWith(groupUid: groupId, meetingId: meetingId);
    }
    on FirebaseException catch (e) {
      rethrow;
    } on Exception catch (e) {
  ...
  static Future addMeeting(String groupId, Meeting meeting) async {
    try {
      var meetingsCollection = db.collection(groups).doc(groupId).collection(meetings);
  ...
  static Future<List<Question>> getQuestions(String groupId, String meetingId, String questionId, Question? lastFetchedQuestion) async {
    try {
      var questionsCollection = db.collection(groups).doc(groupId).collection(meetings).doc(meetingId).collection(questions);
      return  (await questionsCollection.get()).docs.map((v) {
        var question = Question.fromJson(v.data());
        question = question.copyWith(groupUid: groupId, meetingUid: meetingId, questionId: v.id);
        return question;
      }).toList();
    } on FirebaseException catch (e) {
      rethrow;
  ...
  static Future<Question> getQuestion(String groupId, String meetingId, String questionId) async {
    try {
      var group = db.collection(groups).doc(groupId);
      var meeting = group.collection(meetings).doc(meetingId);
      var question = meeting.collection(questions).doc(questionId);
      return Question.fromJson((await question.get()).data()!).copyWith(groupUid: groupId, meetingUid: meetingId, questionId: questionId);
    }
    on FirebaseException catch (e) {
      rethrow;
    } on Exception catch (e) {
  ...
  static Future addQuestion(String groupId, String meetingId, Question question) async {
    try {
      var questionsCollection = db.collection(groups).doc(groupId).collection(meetings).doc(meetingId).collection(questions);
  ...
  static Future<List<Answer>> getAnwers(String groupId, String meetingId, String questionId) async {
    try {
      var answersCollection = db.collection(groups).doc(groupId).collection(meetings).doc(meetingId).collection(questions).doc(questionId).collection(answers);
      return  (await answersCollection.get()).docs.map((v) {
        var answer = Answer.fromJson(v.data());
        answer = answer.copyWith(groupUid: groupId, meetingUid: meetingId, questionUid: questionId, answerId: v.id);
        return answer;
      }).toList();
    } on FirebaseException catch (e) {
      rethrow;
  ...
  static Future<Answer> getAnswer(String groupId, String meetingId, String questionId, String answerId) async {
    try {
      var group = db.collection(groups).doc(groupId);
      var meeting = group.collection(meetings).doc(meetingId);
      var question = meeting.collection(questions).doc(questionId);
      var answer = question.collection(answers).doc(answerId);
      return Answer.fromJson((await answer.get()).data()!).copyWith(groupUid: groupId, meetingUid: meetingId, questionUid: questionId, answerId: answerId);
    }
    on FirebaseException catch (e) {
      rethrow;
    } on Exception catch (e) {
  ...
  static Future addAnswer(String groupId, String meetingId, String questionId, Answer answer) async {
    try {
      var answersCollection = db.collection(groups).doc(groupId).collection(meetings).doc(meetingId).collection(questions).doc(questionId).collection(answers);
